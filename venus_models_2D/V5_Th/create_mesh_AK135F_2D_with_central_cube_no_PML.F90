...
! output a Gnuplot grid or not (set to false when creating a very large mesh for very short periods)
  logical, parameter :: output_gnuplot_grid = .true. ! .true.
...
!                               1 . . . . 5 . . . . 2
  ...
  integer, parameter :: nspec_surf_whole_circle      = 480 * multiplication_factor  ! 512
...
! VENUS EDITS HERE ---- VENUS
  double precision, parameter :: R_EARTH = 6051900.d0

! ! BC values for VENUS (Dumoulin et al. 2017)
  double precision, parameter :: R670   = 5555644.d0
  double precision, parameter :: RCMB   = 3292233.6d0
  double precision, parameter :: RICB   = 1646117.d0
...
! elements 9 noeuds donc on a seulement la moitie des elements
  ...
  print *,'Radius of the mesh = ',R_EARTH
...
! count the number of elements that are in contact with the symmetry axis
  do ispec = 1,nspec
    ...
    ! BC: added first clause because otherwise full mesh was throwing this error
    if (generate_only_half_the_mesh .eqv. .true.) then
      if (i > 0) then
